...
async function syncConversationsToMarkdown() {
  ...
  if (!res.ok) {
    ...
    log(`conversation sync backend error status=${res.status} body=${body.slice(0, 200)}`)
    return
  }
  ...
  const markdown = [
    '# Users Past Session History',
    '',
    '<!-- This file is generated by the Mira Claude plugin. -->',
    ...
    ...sessions.map((s) => {
      ...
      const end = s.end_time ?? '(ongoing)'

      return [
        `<!-- mira-session-id: ${s.id} -->`,
        `## ${start} - ${title}`,
        '',
        `Session ID: ${s.id}`,
        `Time: ${start} -> ${end}`,
  ...
  const dir = join(homedir(), '.mira')
  mkdirSync(dir, { recursive: true })
  writeFileSync(join(dir, 'conversations.md'), markdown, 'utf8')

  log(`conversation sync OK sessions=${sessions.length}`)
}
...
const mcp = new Server(
  { name: 'mira', version: '0.1.0' },
  {
    ...
    instructions:
      ...
      'use `~/.mira/conversations.md` file in the user\'s home directory. run grep, cat, or other commands to search the file. ' +
